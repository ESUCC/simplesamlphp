...
class sspmod_authgoogle_Auth_Source_Google extends SimpleSAML_Auth_Source {
	...
	const MAX_TOKEN_LIFETIME_SECS = 7200;
	...
	public function __construct($info, $config) {
		...
		$this->esuccCdn = $config['esucc-cdn'];
	...
	public function finalStep(&$state) {
		...
		*/
		...
		$attributes['esucc-cdn'] = array($this->esuccCdn);
	...
	*/
	  private function verifySignedJwtWithCerts(
		...
		SimpleSAML_Logger::debug('max_expiry = ' . $max_expiry);
		...
		SimpleSAML_Logger::debug('SI debug test 0');

		if ($exp > $now + $max_expiry) {
		  SimpleSAML_Logger::debug('SI debug test 0.1');
		  throw new Google_Auth_Exception(
			  sprintf("Expiration time too far in future: %s", $json_body)
		  );
		}
		SimpleSAML_Logger::debug('SI debug test 1');
		...
		SimpleSAML_Logger::debug('SI debug test 2');
		if ($now > $latest) {
		  throw new Google_Auth_Exception(
			  sprintf(
		...
		SimpleSAML_Logger::debug('SI debug test 3');
		...
		SimpleSAML_Logger::debug('SI debug test 4');
		// Check audience
		$aud = $payload["aud"];
		...
		SimpleSAML_Logger::debug('SI debug test 5');
	  }
